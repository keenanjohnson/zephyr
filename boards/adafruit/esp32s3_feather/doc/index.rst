...
The Adafruit ESP32-S3 Feather is a powerful development board based on the ESP32-S3 system-on-chip (SoC) from Espressif. 
It is designed for low-power IoT applications and features integrated 2.4 GHz Wi-Fi and Bluetooth Low Energy (Bluetooth LE) capabilities.
The board is equipped with a dual-core Xtensa LX7 microprocessor that can run up to 240MHz, and an additional low power coprocessor. 
It also includes 512KB of SRAM and 384KB of ROM for robust application support.
...
ESP32-S3 Feather includes the following features:
...
ESP32S3-Feather allows 2 different applications to be executed in ESP32-S3 SoC. Due to its dual-core
...
Current Zephyr's ESP32-S3-Feather board supports the following features:
...
The usual ``flash`` target will work with the ``esp32s3_feather`` board
